(()=>{
  ...
  function decorate(){document.querySelectorAll('#workCatalogGrid .cargo-card').forEach(card=>{
    ...
    wrap.innerHTML='<button type="button" class="cargo-compat-toggle" aria-expanded="false" aria-controls="'+id+'"><span>VER CARGAS COMPATÍVEIS</span><span class="arrow">⌄</span></button><div class="cargo-compat-panel" id="'+id+'" hidden><span class="cargo-compat-title">O QUE CONTA NESTE TRABALHO</span>'+(actual.length?'<span class="cargo-compat-source">JÁ VISTAS NO ETS2 PELO GAT TELEMETRIA</span><div class="cargo-compat-chips">'+chips(actual,true)+'</div>':'')+'<span class="cargo-compat-source">TIPOS RECONHECIDOS</span><div class="cargo-compat-chips">'+chips(examples,false)+'</div><span class="cargo-compat-note">A lista verde vem das cargas que o GAT Telemetria encontrou de verdade no ETS2. Ela aumenta conforme os motoristas jogam.</span></div>';
    const btn=wrap.querySelector('button'),panel=wrap.querySelector('.cargo-compat-panel');btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const open=btn.getAttribute('aria-expanded')==='true';btn.setAttribute('aria-expanded',String(!open));panel.hidden=open});body.appendChild(wrap);
  })}
