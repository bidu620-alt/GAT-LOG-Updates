(()=>{
  ...
  const stem=x=>{let s=String(x||'');if(s.length>4&&s.endsWith('s'))s=s.slice(0,-1);return s};
  const words=v=>norm(v).split(' ').map(stem).filter(x=>x.length>2);
...
`;
  ...
  function cargoScore(item,cargo){
    ...
    let best=0;
    const at=new Set(words(actual));
    for(const c of candidates){
      if(actual===c)best=Math.max(best,100);
      else if(actual.includes(c)||c.includes(actual))best=Math.max(best,88);
      else{
        const ct=new Set(words(c));
        let hit=0;
        for(const x of at){
          if(ct.has(x)){hit++;continue}
          if([...ct].some(y=>x.length>=5&&y.length>=5&&(x.startsWith(y)||y.startsWith(x))))hit++;
        }
  ...
  function provisionalSuggestion(pick){
    if(pick?.score>0)return pick;
    const available=items.find(x=>!x.completed)||items[0];
    return available?{item:available,score:0}:pick;
  }
  ...
  function renderSuggestion(){
    ...
    if((m&&!adminTest())||!own()||!freshNow||!liveOnJob(t)||!cargo||!items.length){box.className='work-telemetry-suggestion';box.innerHTML='';return}
    const rawPick=bestSuggestion(cargo),pick=provisionalSuggestion(rawPick),src=liveSource(t),dst=liveDestination(t);
    if(!pick){box.className='work-telemetry-suggestion';box.innerHTML='';return}
    const confident=pick.score>=35,approximate=!confident;
    const card=[...document.querySelectorAll('.cargo-card')].find(x=>x.dataset.workId===String(pick.item.id||''));
    if(card)card.classList.add('gat-suggested');
    box.className='work-telemetry-suggestion show';
    const label=confident?'Trabalho sugerido: '+esc(pick.item.title||pick.item.category||'Trabalho GAT'):'Sugestão provisória: '+esc(pick.item.title||pick.item.category||'Trabalho GAT');
    const note=confident?'Compatibilidade '+pick.score+'% • confirme para marcar este trabalho.':'Carga ainda não cadastrada com segurança. Use esta sugestão provisória para não perder a viagem; depois a carga pode ser adicionada à categoria correta no catálogo.';
    box.innerHTML=
      ...
      '<div class="suggest-match"><div><strong>'+label+'</strong><small>'+note+'</small></div><button type="button" id="workSuggestionConfirm">'+(approximate?'USAR SUGESTÃO PROVISÓRIA':'MARCAR ESTE TRABALHO')+'</button></div>';
