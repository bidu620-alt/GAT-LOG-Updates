...
`;
  ...
  function parseDelivery(d){
    ...
    const speed=Math.max(0,n(d?.speed_penalty_xp)||fines*3),cargo=Math.max(0,n(d?.cargo_penalty_xp)),truck=Math.max(0,n(d?.truck_penalty_xp)),penalty=Math.max(0,n(d?.penalty_xp)||speed+cargo+truck),bonus=Math.max(0,n(d?.perfect_bonus_xp));
    const final=(d&&Object.prototype.hasOwnProperty.call(d,'xp_awarded')&&Number.isFinite(Number(d.xp_awarded)))?Math.max(0,Number(d.xp_awarded)):Math.max(0,base-penalty+bonus);
    return {d,distance,base,fines,speed,cargo,truck,penalty,bonus,final,cargoDamage:Math.max(0,n(d?.cargo_damage_pct)),truckDamage:Math.max(0,n(d?.truck_damage_delta_pct)),perfect:!!d?.perfect_trip||bonus>0};
  }
  ...
  function fallbackSafety(list){
    ...
    return {deliveries:p.length,speed_fines:fines,penalty_xp:lost,perfect_trips:perfect,clean_trips:clean,no_fine_km:noFineKm,avg_cargo_damage_pct:avgC,avg_truck_damage_pct:avgT,score};
  }
  ...
  function render(){
    ensureTabs();const p=currentProfile();if(!p)return;const history=Array.isArray(p.deliveries)?p.deliveries:[],parsed=history.map(parseDelivery),st=p.safety||fallbackSafety(history);
    const infra=document.getElementById('gat-infractions-body');if(infra){
      const bad=[...parsed].reverse().filter(x=>x.penalty>0);
      infra.innerHTML=`<div class="gat-safety-score"><div><span>ÍNDICE DE DIREÇÃO SEGURA</span><strong>${n(st.score).toLocaleString('pt-BR',{maximumFractionDigits:1})}</strong></div><span>0 a 100</span></div><div class="gat-game-grid"><article class="gat-game-card bad"><small>MULTAS DE VELOCIDADE</small><b>${fmt(st.speed_fines)}</b></article><article class="gat-game-card bad"><small>XP PERDIDO</small><b>-${fmt(st.penalty_xp)}</b></article><article class="gat-game-card good"><small>VIAGENS LIMPAS</small><b>${fmt(st.clean_trips)}</b></article><article class="gat-game-card"><small>DANO MÉDIO DA CARGA</small><b>${pct(st.avg_cargo_damage_pct)}</b></article><article class="gat-game-card"><small>DANO MÉDIO DO CAMINHÃO</small><b>${pct(st.avg_truck_damage_pct)}</b></article></div><div class="gat-game-list">${bad.length?bad.slice(0,30).map(x=>`<div class="gat-game-row"><strong>${esc2((x.d?.source||'—')+' → '+(x.d?.destination||'—'))}</strong><span>${km2(x.distance)}</span><span class="${x.fines?'bad':''}">${x.fines} multa${x.fines===1?'':'s'}</span><span class="${x.cargo?'bad':''}">Carga ${pct(x.cargoDamage)}</span><span class="${x.truck?'bad':''}">Caminhão +${pct(x.truckDamage)}</span></div>`).join(''):'<div class="gat-game-empty">Nenhuma infração registrada. Continue assim.</div>'}</div>`;
    }
    const stats=document.getElementById('gat-statistics-body');if(stats){
      const longest=parsed.reduce((a,b)=>b.distance>(a?.distance||0)?b:a,null),avg=parsed.length?parsed.reduce((s,x)=>s+x.distance,0)/parsed.length:0,cargoMap=new Map();for(const x of parsed){const c=String(x.d?.cargo||'Carga');cargoMap.set(c,(cargoMap.get(c)||0)+1)}let favorite='—',favN=0;for(const [c,v] of cargoMap){if(v>favN){favorite=c;favN=v}}
      stats.innerHTML=`<div class="gat-game-grid"><article class="gat-game-card blue"><small>KM TOTAL</small><b>${km2(p.total_km)}</b></article><article class="gat-game-card"><small>MÉDIA POR ENTREGA</small><b>${km2(avg)}</b></article><article class="gat-game-card"><small>MAIOR VIAGEM</small><b>${longest?km2(longest.distance):'—'}</b></article><article class="gat-game-card gold"><small>VIAGENS PERFEITAS</small><b>${fmt(st.perfect_trips)}</b></article><article class="gat-game-card good"><small>KM SEM MULTA</small><b>${km2(st.no_fine_km)}</b></article></div><div class="gat-game-grid"><article class="gat-game-card"><small>CARGA MAIS TRANSPORTADA</small><b style="font-size:14px">${esc2(favorite)}</b></article><article class="gat-game-card"><small>ENTREGAS</small><b>${fmt(p.total_deliveries)}</b></article><article class="gat-game-card"><small>XP TOTAL</small><b>${fmt(p.xp)}</b></article><article class="gat-game-card bad"><small>PENALIDADES ACUMULADAS</small><b>-${fmt(st.penalty_xp)} XP</b></article><article class="gat-game-card gold"><small>TAXA PERFEITA</small><b>${pct(n(st.deliveries)?n(st.perfect_trips)/n(st.deliveries)*100:0)}</b></article></div>`;
    }
