...
.gat-admin-driving-summary{margin:14px 0}.gat-admin-driving-summary h3{margin:4px 0 12px;font-size:18px}.gat-admin-driving-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:8px}.gat-admin-driving-grid article{border:1px solid #1d3044;border-radius:13px;background:#0b141d;padding:12px}.gat-admin-driving-grid small{display:block;color:#71849a;font-size:8px;font-weight:950}.gat-admin-driving-grid b{display:block;margin-top:6px;font-size:17px}.gat-admin-driving-grid .bad b{color:#ff91a7}.gat-admin-driving-grid .good b{color:#60dcb1}.gat-admin-driving-grid .blue b{color:#75bdff}.gat-admin-xp-breakdown{display:flex;flex-wrap:wrap;gap:5px;margin-top:5px}.gat-admin-xp-chip{border:1px solid #2a3d52;border-radius:7px;padding:4px 6px;font-size:7px;font-weight:900;color:#9eb1c5;background:#0e1822;white-space:nowrap}.gat-admin-xp-chip.bad{border-color:#5a2b36;background:#1b1117;color:#ff91a7}.gat-admin-xp-chip.good{border-color:#205a49;background:#0c211b;color:#70dbb5}.gat-admin-xp-final{font-size:12px;font-weight:950;color:#7fc4ff;white-space:nowrap}.gat-admin-adjust-xp{border:1px solid #3978ad!important;background:#10263a!important;color:#89c8ff!important}.gat-admin-adjust-xp:hover{background:#16344f!important}.gat-admin-audit-note{display:block;margin-top:4px;color:#778ba1;font-size:7px;white-space:nowrap}.detail-history-table.audit-mode{min-width:1280px}.detail-history-table.audit-mode th,.detail-history-table.audit-mode td{vertical-align:top}@media(max-width:900px){.gat-admin-driving-grid{grid-template-columns:1fr 1fr}}`;
  ...
  function ensureSummary(){
    ...
    sec.innerHTML='<span class="eyebrow">AUDITORIA DE CONDUÇÃO</span><h3>Infrações e XP</h3><div id="gatAdminDrivingGrid" class="gat-admin-driving-grid"></div>';
  ...
  function parse(d){
    ...
    const penalty=Math.max(0,num(d?.penalty_xp)||speed+cargo+truck),final=(d&&Object.prototype.hasOwnProperty.call(d,'xp_awarded')&&Number.isFinite(Number(d.xp_awarded)))?Math.max(0,Number(d.xp_awarded)):Math.max(0,base-penalty);
    return {distance,base,fines,speed,cargo,truck,penalty,final,cargoDamage:Math.max(0,num(d?.cargo_damage_pct)),truckDamage:Math.max(0,num(d?.truck_damage_delta_pct))};
  }

  function renderSummary(history){
    ensureSummary();const root=document.getElementById('gatAdminDrivingGrid');if(!root)return;
    const list=(Array.isArray(history)?history:[]).map(parse),fines=list.reduce((s,x)=>s+x.fines,0),lost=list.reduce((s,x)=>s+x.penalty,0),clean=list.filter(x=>x.penalty===0).length;
    ...
    root.innerHTML=`<article class="bad"><small>MULTAS</small><b>${fmt(fines)}</b></article><article class="bad"><small>XP PERDIDO</small><b>-${fmt(lost)}</b></article><article class="good"><small>VIAGENS LIMPAS</small><b>${fmt(clean)}</b></article><article><small>DANO MÉDIO CARGA</small><b>${pct(avgC)}</b></article><article class="blue"><small>DANO MÉDIO CAMINHÃO</small><b>${pct(avgT)}</b></article>`;
  }
  ...
  renderDetailHistory=function(history,user){
    ...
    list.forEach(d=>{
      const x=parse(d),id=String(d?.id||d?.receipt_id||''),route=(d?.source||'—')+' → '+(d?.destination||'—'),canEdit=(typeof viewerRole==='undefined'||viewerRole!=='moderator'),manual=x.final!==Math.max(0,x.base-x.penalty);
      const tr=document.createElement('tr');
      tr.innerHTML=`<td>${safe(date(d?.completed_at))}</td><td class="route-cell"><b>${safe(route)}</b><small class="gat-admin-audit-note">${safe(d?.cargo||'Carga')}</small></td><td>${safe(km(d?.distance_km))}<small class="gat-admin-audit-note">${safe(weight(d?.weight_kg))}</small></td><td><b>${fmt(x.base)} XP</b></td><td><div class="gat-admin-xp-breakdown"><span class="gat-admin-xp-chip ${x.speed?'bad':''}">${x.fines} multa${x.fines===1?'':'s'}</span><span class="gat-admin-xp-chip ${x.speed?'bad':''}">${x.speed?'-'+fmt(x.speed):'0'} XP</span></div></td><td><div class="gat-admin-xp-breakdown"><span class="gat-admin-xp-chip ${x.cargo?'bad':''}">Carga ${pct(x.cargoDamage)} ${x.cargo?'-'+fmt(x.cargo)+' XP':''}</span><span class="gat-admin-xp-chip ${x.truck?'bad':''}">Caminhão +${pct(x.truckDamage)} ${x.truck?'-'+fmt(x.truck)+' XP':''}</span></div></td><td><span class="gat-admin-xp-final">${fmt(x.final)} XP</span>${manual?'<small class="gat-admin-audit-note">ajuste manual / divergência</small>':''}</td><td>#${safe(d?.sequence||'—')}</td><td>${canEdit&&id?'<button class="gat-admin-adjust-xp" data-adjust-xp="'+safe(id)+'" data-user="'+safe(user)+'" data-current="'+x.final+'">AJUSTAR XP</button> ':''}${canEdit&&id?'<button data-delete-delivery="'+safe(id)+'" data-delivery-user="'+safe(user)+'">EXCLUIR</button>':'—'}</td>`;
      rows.appendChild(tr);
    });
