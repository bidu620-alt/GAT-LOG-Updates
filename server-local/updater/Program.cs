...
{
    const string PackageUrl="https://raw.githubusercontent.com/bidu620-alt/GAT-LOG-Updates/main/releases/GAT_SERVER_LOCAL_1.0.47.zip";
    ...
    Updater(){Text="GAT Servidor 1.0.47 - cargas e ranking";ClientSize=new Size(590,160);StartPosition=FormStartPosition.CenterScreen;Controls.Add(status);Shown+=async(s,e)=>await Install();}
    ...
    async Task Install(){
        ...
        try{
            ...
            if(version!="1.0.39.0"&&version!="1.0.40.0"&&version!="1.0.41.0"&&version!="1.0.42.0"&&version!="1.0.43.0"&&version!="1.0.44.0"&&version!="1.0.45.0"&&version!="1.0.46.0"&&version!="1.0.47.0")throw new InvalidOperationException("Esta atualizacao foi preparada para as versoes 1.0.39 a 1.0.47. Versao encontrada: "+version);
            ...
            status.Text="Instalando Central 1.0.47 e ajustando o ranking...";
            ...
            MessageBox.Show(this,"GAT Servidor 1.0.47 instalado.\r\nA janela inicial da telemetria foi ampliada para evitar rejeicao por pacote transitorio de danos ao pegar uma carga.\r\nA entrega perdida do lapeal67 (Mower Conditioner Krone BiG M 450, Malaga > A Coruna, 1.135 km) sera recuperada automaticamente e de forma idempotente no ranking.\r\nO reparo cria backup antes de alterar o banco e nao mexe na viagem atual do motorista.\r\nBanco, pontos, entregas, contas, senhas e PCs vinculados foram preservados.","Atualizacao concluida",MessageBoxButtons.OK,MessageBoxIcon.Information);Close();
        }catch(Exception ex){
