...
test('carga e classificada sem motorista escolher trabalho; desconhecida vai para moderacao e aprende',async t=>{
 ...
 p=db.sql.prepare("SELECT monthly_completed,total_deliveries,points FROM profiles WHERE user='biduzao'").get();assert.equal(p.monthly_completed,1);assert.equal(p.total_deliveries,2);assert.equal(p.points,200);
 ...
 const before=db.sql.prepare("SELECT points,xp,total_deliveries FROM profiles WHERE user='biduzao'").get();const classified=await admin('/api/site/admin/classify',{queue_id:queue.id,work_id:'heavy'});assert.equal(classified.r.status,200,JSON.stringify(classified.data));assert.equal(classified.data.counted,true);
 p=db.sql.prepare("SELECT monthly_completed,total_deliveries,points,xp FROM profiles WHERE user='biduzao'").get();assert.equal(p.monthly_completed,2);assert.equal(p.total_deliveries,before.total_deliveries);assert.equal(p.points,before.points);assert.equal(p.xp,before.xp);
