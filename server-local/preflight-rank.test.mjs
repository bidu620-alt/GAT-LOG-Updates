...
function delivered(){return loaded({gat_job_event:'delivered',gat_job_state:'idle',job_latched:false,on_job:false,cargo_name:'',mass_kg:0,remaining_km:0,truck:{odometer:10002},gameplay:{jobDelivered:true,jobCancelled:false,jobDeliveredDetails:{distanceKm:600,revenue:1000,earnedXp:10,cargoDamage:0}}});}
...
test('idle prova plugin atual e permite pacote inicial perder os cinco danos do caminhao sem condenar a viagem',async t=>{
 ...
 await send(loaded({remaining_km:599}));mission=JSON.parse(db.sql.prepare("SELECT current_mission_json FROM profiles WHERE user='driverpre'").get().current_mission_json);assert.equal(mission.rank_guard.valid_samples,1);assert.notEqual(mission.rank_guard.incompatible_damage_plugin,true);
 await send(loaded({remaining_km:598,truck_engine_damage_pct:.1}));mission=JSON.parse(db.sql.prepare("SELECT current_mission_json FROM profiles WHERE user='driverpre'").get().current_mission_json);assert.equal(mission.rank_guard.reason,null);assert.equal(mission.rank_guard.valid_samples,2);assert.ok(mission.rank_guard.verified_at);
