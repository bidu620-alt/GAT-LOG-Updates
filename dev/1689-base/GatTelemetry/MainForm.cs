...
internal sealed partial class MainForm : Form
{
	...
	private void BuildUi()
	{
		...
		Application.ApplicationExit += delegate { DashMediaBridge.Stop(); };
	...
	private SpeechSynthesizer EnsureVoice()
	{
		...
		_voice.Volume = VoiceVolume062Value();
		_voice.Rate = 0;
		return _voice;
	}
	...
	private bool SpeakGat(string text, string logLabel)
	{
		try
		{
			if (VoiceHandleLegacySpeak062(text, logLabel)) return true;
			EnsureVoice().Volume = VoiceVolume062Value();
	...
	private async Task CaptureTripJournalAsync()
	{
		if (!((DateTime.UtcNow - _lastTripCapture).TotalMilliseconds < 850.0))
		{
			...
			if (jObject != null)
			{
				ObserveVoiceTelemetry062(jObject);
...
private static void CopyValue(JObject a, JObject b, string output, params string[] paths)
	...
	private void UpdateTelemetryDisplay(TelemetryDisplay d)
	{
		...
		ObserveDamageDisplay064(d.CargoDamage, d.EngineDamage, d.TransmissionDamage, d.CabinDamage, d.ChassisDamage, d.WheelsDamage, d.TrailerDamage, d.Speed);
		if (lblDamage != null)
		{
