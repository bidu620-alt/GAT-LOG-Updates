...
$ManifestUrl='https://raw.githubusercontent.com/bidu620-alt/GAT-LOG-Updates/main/manifests/servidor.json'

function Msg([string]$Text,[string]$Title='GAT LOG BETA | Atualizacao'){
...
try{
    ...
    $tmp=Join-Path $env:TEMP ('gatlog_patch_servidor_'+[Guid]::NewGuid().ToString('N')+'.ps1')
...
}catch{Msg ("Nao foi possivel iniciar a atualizacao.`r`n`r`n"+$_.Exception.Message) 'GAT LOG BETA | Falha na atualizacao'}
