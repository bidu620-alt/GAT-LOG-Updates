...
if (-not $main -or -not $hub -or -not $voice) { throw 'Fonte 1.0.68 incompleto para aplicar VOZ LIMPA 1.0.68.6.' }
...
function Replace-Between1681([string]$text, [string]$startMarker, [string]$endMarker, [string]$replacement) {
    ...
    if ($s -lt 0) { throw "Inicio nao encontrado 1.0.68.6: $startMarker" }
    $e = $text.IndexOf($endMarker, $s + $startMarker.Length, [StringComparison]::Ordinal)
    if ($e -lt 0) { throw "Fim nao encontrado 1.0.68.6: $endMarker" }
...
$mainText = $mainText.Replace('CurrentVersion = "1.0.68.0"', 'CurrentVersion = "1.0.68.6"')
$mainText = $mainText.Replace('Text = "Cliente 1.0.68"', 'Text = "Cliente 1.0.68.6"')
$mainText = $mainText.Replace('HUB 1.0.68:', 'HUB 1.0.68.6:')
$hubText = [regex]::Replace($hubText, 'Text\s*=\s*"GAT Telemetria BETA 1\.0\.68"', 'Text = "GAT Telemetria BETA 1.0.68.6 VOZ LIMPA"', 1)
$hubText = $hubText.Replace('Cliente 1.0.68 TESTE', 'Cliente 1.0.68.6 VOZ LIMPA TESTE')
if ($hub45Text) {
    $hub45Text = $hub45Text.Replace('GAT Telemetria BETA 1.0.68', 'GAT Telemetria BETA 1.0.68.6 VOZ LIMPA')
    $hub45Text = $hub45Text.Replace('Cliente: 1.0.68 TESTE', 'Cliente: 1.0.68.6 VOZ LIMPA TESTE')
}

# A preferencia de voz do DASH nao controla mais o motor de audio do Telemetria.
$voiceText = $voiceText.Replace('if (o["voice"] != null) _voiceMuted062 = !Convert.ToBoolean(o["voice"]);', '_voiceMuted062 = false; // 1.0.68.6: voz pertence ao Telemetria, nao ao DASH.')

# Toda fala solicitada pelo DASH e consumida sem reproduzir audio.
$dashHandler = @'
    private bool VoiceHandleDashSpeak062(string text)
    {
        // 1.0.68.6 VOZ LIMPA: o DASH e somente visual.
        // Nenhum "speak" vindo do HTML pode acionar WAV ou TTS.
        return true;
    }
'@
...
$dashSettings = @'
    ...
    {
        // 1.0.68.6: configuracao de voz e local do GAT Telemetria.
    }
'@
...
$legacyHandler = @'
    ...
    {
        // 1.0.68.6 VOZ LIMPA: evita qualquer segunda voz/TTS legado.
        return true;
    }
'@
...
# Filtro central: somente inicio/fim de trabalho usam grupos; velocidade usa o WAV numerico direto.
$groupHead = @'
...
$fields = @'
    ...
    private System.Media.SoundPlayer _voiceWavePlayer1686;
...
$cleanMethods = @'
        ...
        _voiceRoadTimer1683.Tick += async delegate
        {
            ...
            try
            {
                ...
                if (!_voiceRoadConnectedLogged1683)
                {
                    ...
                    ClientStore.Log("VOZ LIMPA 1.0.68.6 TruckSim direto conectado");
                }
            }
            catch (Exception ex1683)
            {
                if ((DateTime.UtcNow - _voiceRoadLastError1683).TotalSeconds >= 15)
                {
                    ...
                    try { ClientStore.Log("VOZ LIMPA 1.0.68.6 TruckSim indisponivel: " + ex1683.Message); } catch { }
                }
            }
            finally
            {
    ...
    private void VoiceCleanObserveRoad1683(JObject tele, DateTime now)
    {
        ...
        if (due)
        {
            ...
            if (played)
            {
                ...
                ClientStore.Log(
                    "VOZ LIMPA 1.0.68.6 excesso direto TruckSim: limite " + limit.ToString(CultureInfo.InvariantCulture) +
                    " km/h | velocidade " + Math.Round(speed).ToString(CultureInfo.InvariantCulture));
            }
            else
            {
                ClientStore.Log("VOZ LIMPA 1.0.68.6 sem WAV para limite: " + limit + " km/h");
            }
        }
    ...
    private bool VoiceSpeakClean1686(string text)
    {
        if (_voiceMuted062 || _voiceVolume062 <= 0 || string.IsNullOrWhiteSpace(text)) return false;
        try
        {
            var synth = EnsureVoice();
            synth.Volume = Math.Max(1, Math.Min(100, _voiceVolume062));
            synth.SpeakAsyncCancelAll();
            synth.SpeakAsync(text);
            ClientStore.Log("VOZ 1.0.68.6 TTS: " + text);
            return true;
        }
        catch (Exception ex)
        {
            ClientStore.Log("VOZ 1.0.68.6 TTS falhou: " + ex.Message);
            return false;
        }
    }
    ...
    private void VoiceCleanObserve1681(JObject tele, DateTime now)
    {
                ...
                else
                {
                    ...
                    if (due)
                    {
                        ...
                        if (played)
                        {
                            ...
                            _voiceCleanOverspeed1681 = true;
                            ClientStore.Log(
                                "VOZ LIMPA 1.0.68.6 excesso: limite " + limit.ToString(CultureInfo.InvariantCulture) +
                                " km/h | velocidade " + Math.Round(speed).ToString(CultureInfo.InvariantCulture));
                        }
                        else
                        {
        ...
        if (ended)
        {
            if (_voiceCleanJobActive1681 && (now - _voiceCleanLastJobEnd1681).TotalSeconds >= 4)
            {
                ...
                VoiceSpeakClean1686("Trabalho finalizado.");
                ClientStore.Log("VOZ LIMPA 1.0.68.6 trabalho finalizado");
            }
        ...
        if (active)
        {
            ...
            if (newJob && (now - _voiceCleanLastJobStart1681).TotalSeconds >= 4)
            {
                ...
                VoiceSpeakClean1686("Trabalho iniciado.");
                ClientStore.Log("VOZ LIMPA 1.0.68.6 trabalho iniciado");
            }
...
# 1.0.68.6: inicia um polling proprio do TruckSim GPS, sem depender da aba/janela do DASH.
...
if ($hubText -notlike '*VoiceRoadInitialize1683();*') {
    ...
    if (-not $hubText.Contains($voiceInitHub1683)) { throw 'VoiceInitialize062 do Hub nao encontrado para 1.0.68.6.' }
...
$voiceText = [regex]::Replace(
    ...
    'test.Click += delegate { _voiceMuted062 = false; if (_voiceVolume062 <= 0) _voiceVolume062 = 80; if (!VoicePlaySpeedLimit065(80)) MessageBox.Show("Falha ao tocar C:\\Program Files\\GAT Telemetria\\voz\\limites\\limite_080.wav. Verifique a instalacao.", "Voz GAT", MessageBoxButtons.OK, MessageBoxIcon.Error); };',
    1
)
...
# 1.0.68.6: configuracao de volume tambem e limpa e independente das versoes antigas.
...
$newVolumeFile1685 = '    private static string VoiceVolumeFile062() { return Path.Combine(VoiceDataDir062(), "voice-volume-clean-1.0.68.6.txt"); }'
...
if ($voiceText.Contains($loadCall1685) -and $voiceText -notlike '*1.0.68.6: volume limpo*') {
    $voiceText = $voiceText.Replace(
        $loadCall1685,
        $loadCall1685 + "`r`n        _voiceMuted062 = false;`r`n        if (_voiceVolume062 <= 0) _voiceVolume062 = 80; // 1.0.68.6: volume limpo"
    )
}
...
# 1.0.68.6: pasta de voz oficial fica junto do aplicativo.
...
$voiceText = $voiceText.Replace('        VoiceTryAutoImport062();', '        // 1.0.68.6: autoimport legado desativado.')
$voiceText = $voiceText.Replace('        VoiceTryAutoImportSpeed065();', '        // 1.0.68.6: autoimport de limite legado desativado.')
...
$newSpeed1684 = @'
    ...
    {
        if (_voiceMuted062 || _voiceVolume062 <= 0) return false;

        string file = VoiceFindSpeed166(limit);
        if (string.IsNullOrWhiteSpace(file) || !File.Exists(file))
        {
            ClientStore.Log("VOZ 1.0.68.6 arquivo ausente: limite_" + limit.ToString("D3", CultureInfo.InvariantCulture) + ".wav");
            return false;
        }

        try
        {
            try { if (_voiceWavePlayer1686 != null) _voiceWavePlayer1686.Stop(); } catch { }
            _voiceWavePlayer1686 = new System.Media.SoundPlayer(file);
            _voiceWavePlayer1686.Load();
            _voiceWavePlayer1686.Play();
            ClientStore.Log("VOZ 1.0.68.6 WAV tocando: " + Path.GetFileName(file));
            return true;
        }
        catch (Exception ex)
        {
            ClientStore.Log("VOZ 1.0.68.6 WAV falhou: " + ex.Message);
            return false;
        }
    }
...
if ($replacedSpeed1684 -eq $voiceText) { throw 'VoicePlaySpeedLimit065 nao encontrado para limpeza 1.0.68.6.' }
...
if ($obsStart1684 -ge 0) {
    ...
    if ($speedComment1684 -ge 0 -and $jobComment1684 -gt $speedComment1684) {
        ...
        $voiceText = $voiceText.Substring(0, $speedComment1684) +
            '        // 1.0.68.6: velocidade e processada apenas pelo polling direto do TruckSim GPS.' + $nl1684 + $nl1684 +
...
$voiceText = $voiceText.Replace(
    ...
    'Motor de voz local • WAV oficial em C:\\Program Files\\GAT Telemetria\\voz\\limites • DASH somente visual • TESTAR VOZ = limite 80.'
)


foreach ($m in @('CurrentVersion = "1.0.68.6"')) {
...
foreach ($m in @('VoiceRoadInitialize1683','VoiceCleanObserve1681','VoiceCleanObserveRoad1683','VoiceCleanPathNumber1681','VoiceCleanSpeedLimit1681','_voiceRoadTimer1683','127.0.0.1:31377','AppDomain.CurrentDomain.BaseDirectory, "voz", "limites"','voice-volume-clean-1.0.68.6.txt','1.0.68.6: volume limpo','System.Media.SoundPlayer','VOZ 1.0.68.6 WAV tocando','VoiceSpeakClean1686','VoicePlaySpeedLimit065(limit)','VoicePlaySpeedLimit065(80)','VOZ LIMPA: grupos permitidos','trabalho iniciado','trabalho finalizado')) {
...
Write-Host 'GAT Telemetria 1.0.68.6 VOZ LIMPA: voz independente do DASH, somente trabalho + limites + finalizacao.'
