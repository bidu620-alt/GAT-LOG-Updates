...
internal sealed class TruckOverlay041 : Form
{
    ...
    private void BuildUi()
    {
        ...
        _weight.Text = "PESO\r\n—";
        _remaining.Text = "DISTÂNCIA RESTANTE\r\n—";
        ...
        _hint.Text = "Arraste pelo topo • redimensione pelas bordas.";
        _hint.Height = 28;
    ...
    private void LayoutResponsive045()
    {
        ...
        if (showJob)
        {
            _jobPanel.SetBounds(pad, top, usable, 112);
            int half = (usable - gap) / 2;
            _cargo.SetBounds(0, 0, half, 52);
            _destination.SetBounds(half + gap, 0, half, 52);
            _weight.SetBounds(0, 60, half, 52);
            _remaining.SetBounds(half + gap, 60, half, 52);
    ...
    private async Task PollAsync()
    {
        ...
        try
        {
            ...
            _toleranceValue = LoadTolerance045();
            if (speed > 5) _avgSpeed = _avgSpeed <= 0 ? speed : (_avgSpeed * 0.94 + speed * 0.06);
            ...
            _remaining.Text = "DISTÂNCIA RESTANTE\r\n" + (remain > 0 ? Math.Round(remain) + " km" : "—");
            _average.Text = "VELOCIDADE MÉDIA\r\n" + (_avgSpeed > 1 ? Math.Round(_avgSpeed) + " km/h" : "—");
    ...
    {
        if (remainKm <= 0 || avgSpeed < 10) return "—";
    ...
    private static double D(JObject j, params string[] paths)
    {
        foreach (string p in paths)
        {
            try
            {
                ...
                double v;
                if (t != null && double.TryParse(Convert.ToString(t), NumberStyles.Any, CultureInfo.InvariantCulture, out v)) return v;
            }
            catch { }
        }
        return 0;
