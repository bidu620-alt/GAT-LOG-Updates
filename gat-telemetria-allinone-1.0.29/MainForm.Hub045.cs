...
internal sealed partial class MainForm
{
    ...
    private void ApplyHub045()
    {
        ...
        _hub045Applied = true;
        Text = "GAT Telemetria BETA 1.0.45";
    ...
    private async Task RefreshDrivers045(bool force)
    {
        ...
        try
        {
            ...
            try
            {
                PlayersResult r = await _api.GetPlayersAsync(_endpoint);
                if (r != null && r.Ok && r.Players != null)
                    names.AddRange(r.Players.Where(x => !string.IsNullOrWhiteSpace(x)));
            }
            catch { }

            string current = !string.IsNullOrWhiteSpace(_driver) ? _driver : _accountUser;
            if (!string.IsNullOrWhiteSpace(current) && !names.Any(x => string.Equals(x, current, StringComparison.OrdinalIgnoreCase)))
                names.Insert(0, current);
            names = names.Distinct(StringComparer.OrdinalIgnoreCase).Take(64).ToList();
            ...
            {
                bool mine = !string.IsNullOrWhiteSpace(current) && string.Equals(n, current, StringComparison.OrdinalIgnoreCase);
                string status = mine && inRoute ? "● Em rota" : "● Online";
                if (mine && inRoute) routeCount++;
                _homeDrivers045.Rows.Add(
                    n,
                    status,
                    mine ? (string.IsNullOrWhiteSpace(cargo) ? "Sem carga" : cargo) : "—",
                    mine ? (string.IsNullOrWhiteSpace(destination) ? "—" : destination) : "—",
                    mine ? (string.IsNullOrWhiteSpace(speed) ? "0 km/h" : speed) : "—",
                    mine ? (string.IsNullOrWhiteSpace(remaining) ? "—" : remaining) : "—"
                );
            }
            if (_homeDriversCount045 != null)
                _homeDriversCount045.Text = names.Count + " online  •  " + routeCount + " em rota";
        }
        catch { }
