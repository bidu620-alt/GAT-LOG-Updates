...
if (-not $main -or -not $hub -or -not $voice) { throw 'Fonte 1.0.68 incompleto para aplicar VOZ LIMPA 1.0.68.7.' }
...
function Replace-Between1681([string]$text, [string]$startMarker, [string]$endMarker, [string]$replacement) {
    ...
    if ($s -lt 0) { throw "Inicio nao encontrado 1.0.68.7: $startMarker" }
    $e = $text.IndexOf($endMarker, $s + $startMarker.Length, [StringComparison]::Ordinal)
    if ($e -lt 0) { throw "Fim nao encontrado 1.0.68.7: $endMarker" }
...
$mainText = $mainText.Replace('CurrentVersion = "1.0.68.0"', 'CurrentVersion = "1.0.68.7"')
$mainText = $mainText.Replace('Text = "Cliente 1.0.68"', 'Text = "Cliente 1.0.68.7"')
$mainText = $mainText.Replace('HUB 1.0.68:', 'HUB 1.0.68.7:')
$hubText = [regex]::Replace($hubText, 'Text\s*=\s*"GAT Telemetria BETA 1\.0\.68"', 'Text = "GAT Telemetria BETA 1.0.68.7 VOZ LIMPA"', 1)
$hubText = $hubText.Replace('Cliente 1.0.68 TESTE', 'Cliente 1.0.68.7 VOZ LIMPA TESTE')
if ($hub45Text) {
    $hub45Text = $hub45Text.Replace('GAT Telemetria BETA 1.0.68', 'GAT Telemetria BETA 1.0.68.7 VOZ LIMPA')
    $hub45Text = $hub45Text.Replace('Cliente: 1.0.68 TESTE', 'Cliente: 1.0.68.7 VOZ LIMPA TESTE')
}

# A preferencia de voz do DASH nao controla mais o motor de audio do Telemetria.
$voiceText = $voiceText.Replace('if (o["voice"] != null) _voiceMuted062 = !Convert.ToBoolean(o["voice"]);', '_voiceMuted062 = false; // 1.0.68.7: voz pertence ao Telemetria, nao ao DASH.')

# Toda fala solicitada pelo DASH e consumida sem reproduzir audio.
$dashHandler = @'
    private bool VoiceHandleDashSpeak062(string text)
    {
        // 1.0.68.7 VOZ LIMPA: o DASH e somente visual.
...
$dashSettings = @'
    ...
    {
        // 1.0.68.7: configuracao de voz e local do GAT Telemetria.
    }
'@
...
$legacyHandler = @'
    ...
    {
        // 1.0.68.7 VOZ LIMPA: evita qualquer segunda voz/TTS legado.
        return true;
    }
'@
...
$cleanMethods = @'
    ...
    private static int VoiceCleanSpeedLimit1681(double roadLimit)
    {
        ...
        // TruckSim GPS devolve limites britanicos convertidos para km/h:
        // 20 mph=32, 30 mph=48, 40 mph=64, 50 mph=80, 60 mph=97, 70 mph=113.
        // O limite REAL continua sendo usado para decidir excesso; aqui escolhemos apenas o arquivo de voz.
        if (SpeedLimitValues065.Contains(nearest) && Math.Abs(roadLimit - nearest) <= 5.1) return nearest;
        return 0;
    }
        ...
        _voiceRoadTimer1683.Tick += async delegate
        {
            ...
            try
            {
                ...
                if (!_voiceRoadConnectedLogged1683)
                {
                    ...
                    ClientStore.Log("VOZ LIMPA 1.0.68.7 TruckSim direto conectado");
                }
            }
            catch (Exception ex1683)
            {
                if ((DateTime.UtcNow - _voiceRoadLastError1683).TotalSeconds >= 15)
                {
                    ...
                    try { ClientStore.Log("VOZ LIMPA 1.0.68.7 TruckSim indisponivel: " + ex1683.Message); } catch { }
                }
            }
            finally
            {
    ...
    private void VoiceCleanObserveRoad1683(JObject tele, DateTime now)
    {
        ...
        if (due)
        {
            ...
            if (played)
            {
                ...
                ClientStore.Log(
                    "VOZ LIMPA 1.0.68.7 excesso direto TruckSim: limite real " + Math.Round(roadLimit).ToString(CultureInfo.InvariantCulture) +
                    " km/h | voz " + limit.ToString(CultureInfo.InvariantCulture) +
                    " | velocidade " + Math.Round(speed).ToString(CultureInfo.InvariantCulture));
            }
            else
            {
                ClientStore.Log("VOZ LIMPA 1.0.68.7 sem MP3: limite real " + Math.Round(roadLimit) + " | voz mapeada " + limit);
            }
        }
    ...
    private void VoiceCleanObserve1681(JObject tele, DateTime now)
    {
                ...
                else
                {
                    ...
                    if (due)
                    {
                        ...
                        if (played)
                        {
                            ...
                            _voiceCleanOverspeed1681 = true;
                            ClientStore.Log(
                                "VOZ LIMPA 1.0.68.7 excesso: limite " + limit.ToString(CultureInfo.InvariantCulture) +
                                " km/h | velocidade " + Math.Round(speed).ToString(CultureInfo.InvariantCulture));
                        }
                        else
                        {
                            ClientStore.Log("VOZ LIMPA 1.0.68.7 sem MP3 para limite: " + limit + " km/h");
                        }
                    }
        ...
        if (ended)
        {
            if (_voiceCleanJobActive1681 && (now - _voiceCleanLastJobEnd1681).TotalSeconds >= 4)
            {
                ...
                ClientStore.Log("VOZ LIMPA 1.0.68.7 trabalho finalizado");
            }
        ...
        if (active)
        {
            ...
            if (newJob && (now - _voiceCleanLastJobStart1681).TotalSeconds >= 4)
            {
                ...
                ClientStore.Log("VOZ LIMPA 1.0.68.7 trabalho iniciado");
            }
...
# 1.0.68.7: inicia um polling proprio do TruckSim GPS, sem depender da aba/janela do DASH.
...
if ($hubText -notlike '*VoiceRoadInitialize1683();*') {
    ...
    if (-not $hubText.Contains($voiceInitHub1683)) { throw 'VoiceInitialize062 do Hub nao encontrado para 1.0.68.7.' }
...
if ($voiceText.Contains($observeNeedle) -and $voiceText -notlike '*VoiceCleanObserve1681(tele, now);*') {
    $voiceText = $voiceText.Replace($observeNeedle, $observeNeedle + "`r`n        VoiceCleanObserve1681(tele, now);`r`n        return; // 1.0.68.7: bloqueia chuva/dano/random/fuel/arriving antigos.")
}
...
# 1.0.68.7: configuracao de volume tambem e limpa e independente das versoes antigas.
...
$newVolumeFile1685 = '    private static string VoiceVolumeFile062() { return Path.Combine(VoiceDataDir062(), "voice-volume-clean-1.0.68.7.txt"); }'
...
if ($voiceText.Contains($loadCall1685) -and $voiceText -notlike '*1.0.68.7: volume limpo*') {
    $voiceText = $voiceText.Replace(
        $loadCall1685,
        $loadCall1685 + "`r`n        _voiceMuted062 = false;`r`n        if (_voiceVolume062 <= 0) _voiceVolume062 = 80; // 1.0.68.7: volume limpo"
    )
}
...
# 1.0.68.7: pasta de voz oficial fica junto do aplicativo.
...
$voiceText = $voiceText.Replace('        VoiceTryAutoImport062();', '        // 1.0.68.7: autoimport legado desativado.')
$voiceText = $voiceText.Replace('        VoiceTryAutoImportSpeed065();', '        // 1.0.68.7: autoimport de limite legado desativado.')
...
$newSpeed1684 = @'
    private bool VoicePlaySpeedLimit065(int limit)
    {
        ...
        {
            ClientStore.Log("VOZ 1.0.68.7 arquivo ausente: limite_" + limit.ToString("D3", CultureInfo.InvariantCulture) + ".mp3");
            return false;
        }

        bool ok = VoicePlayFile166(file, true);
        if (ok)
            ClientStore.Log("VOZ 1.0.68.7 tocando: " + Path.GetFileName(file));
        return ok;
    }
...
if ($replacedSpeed1684 -eq $voiceText) { throw 'VoicePlaySpeedLimit065 nao encontrado para limpeza 1.0.68.7.' }
...
if ($obsStart1684 -ge 0) {
    ...
    if ($speedComment1684 -ge 0 -and $jobComment1684 -gt $speedComment1684) {
        ...
        $voiceText = $voiceText.Substring(0, $speedComment1684) +
            '        // 1.0.68.7: velocidade e processada apenas pelo polling direto do TruckSim GPS.' + $nl1684 + $nl1684 +
...
foreach ($m in @('CurrentVersion = "1.0.68.7"')) {
...
foreach ($m in @('VoiceRoadInitialize1683','VoiceCleanObserve1681','VoiceCleanObserveRoad1683','VoiceCleanPathNumber1681','VoiceCleanSpeedLimit1681','Math.Abs(roadLimit - nearest) <= 5.1','_voiceRoadTimer1683','127.0.0.1:31377','AppDomain.CurrentDomain.BaseDirectory, "voz", "limites"','voice-volume-clean-1.0.68.7.txt','1.0.68.7: volume limpo','VOZ 1.0.68.7 tocando','VoicePlaySpeedLimit065(limit)','VoicePlaySpeedLimit065(80)','VOZ LIMPA: grupos permitidos','trabalho iniciado','trabalho finalizado','bloqueia chuva/dano/random/fuel/arriving antigos')) {
...
Write-Host 'GAT Telemetria 1.0.68.7 VOZ LIMPA: voz independente do DASH, somente trabalho + limites + finalizacao.'
