...
# 1.0.67 TESTE: gerenciador de pacotes de voz pelo proprio aplicativo.
...
# Novo painel de Voz e Alertas.
$patternSettings = '(?s)    private void BuildVoiceSettings062\(Panel p\)\s*\{.*?\r?\n    \}\r?\n\r?\n    private void VoiceRefreshUi062\(\)'
$newSettings = @'
    ...
    private void VoiceRefreshUi062()
'@
$replacedSettings = [regex]::Replace($voiceText, $patternSettings, $newSettings, 1)
if ($replacedSettings -eq $voiceText) { throw 'BuildVoiceSettings062 nao encontrado para 1.0.67.' }
$voiceText = $replacedSettings

# Status do pacote ativo.
$patternRefresh = '(?s)    private void VoiceRefreshUi062\(\)\s*\{.*?\r?\n    \}\r?\n\r?\n    private void VoiceSetVolume062'
$newRefresh = @'
    private void VoiceRefreshUi062()
    {
        try
        {
            ...
            if (_voiceStatusLabel062 != null)
            {
                ...
                try
                {
                    string er = VoiceExtrasRoot167();
                    if (Directory.Exists(er)) extras = Directory.GetFiles(er, "*.mp3", SearchOption.AllDirectories).Length;
                }
                catch { }
    ...
    private void VoiceSetVolume062
'@
$replacedRefresh = [regex]::Replace($voiceText, $patternRefresh, $newRefresh, 1)
if ($replacedRefresh -eq $voiceText) { throw 'VoiceRefreshUi062 nao encontrado para 1.0.67.' }
$voiceText = $replacedRefresh

# Insere o gerenciador antes das configuracoes sincronizadas do DASH.
$insertBefore = '    private void VoiceSyncDashSetting062(JObject message)'
if ($voiceText.Contains($insertBefore) -and $voiceText -notlike '*VoiceReplacePackage167*') {
$manager = @'
    ...
    private static string VoiceBackupRoot167()
    {
        return Path.Combine(VoiceDataDir062(), "voicepack167-backup-latest");
    }
    ...
    private static void VoiceCopyDir167(string source, string dest)
    {
        ...
        foreach (string sub in Directory.GetDirectories(source))
            VoiceCopyDir167(sub, Path.Combine(dest, Path.GetFileName(sub)));
    }
    ...
    private void VoiceReplacePackage167()
    {
        ...
        {
            dlg.Description = "Selecione a pasta do pacote de voz. Ela pode conter as pastas general e limits.";
            ...
            var generalFiles = new List<string>();
            var limitFiles = new List<string>();
            ...
            try
            {
                ...
                foreach (string src in generalFiles)
                {
                    ...
                    foreach (string old in Directory.GetFiles(gd, prefix + "*.mp3"))
                ...
                foreach (string src in limitFiles)
                {
                    ...
                    string dest = Path.Combine(ld, "limite_" + limit.ToString("D3", CultureInfo.InvariantCulture) + ".mp3");
            ...
            {
                MessageBox.Show("Não foi possível substituir a voz: " + ex.Message, "Voz GAT", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
    ...
    private void VoiceAddExtras167()
    {
        using (var dlg = new FolderBrowserDialog())
        {
            dlg.Description = "Selecione a pasta que contém extras/<grupo>/ ou uma pasta com os grupos startup, crash, delivery, random etc.";
            if (dlg.ShowDialog(this) != DialogResult.OK) return;
            string root = dlg.SelectedPath;
            ...
            try
            {
                ...
                if (VoiceGroupAllowed167(selectedName))
                {
                    ...
                    foreach (string src in Directory.GetFiles(extrasSource, "*.mp3"))
                    {
                        File.Copy(src, VoiceUniqueExtra167(destDir, Path.GetFileName(src)), false);
                        added++;
                    }
                }
                else
                {
                    foreach (string group in VoiceGroups167)
                    {
                        ...
                        foreach (string src in Directory.GetFiles(srcDir, "*.mp3"))
                        {
                            File.Copy(src, VoiceUniqueExtra167(destDir, Path.GetFileName(src)), false);
                            added++;
                        }
                ...
                if (added > 0)
                {
                    ...
                    MessageBox.Show(added + " fala(s) extra(s) adicionada(s). Elas passam a participar do sorteio do grupo correspondente.", "Voz GAT", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("Nenhuma fala extra encontrada. Use pastas como extras/crash, extras/delivery, extras/random etc.", "Voz GAT", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show("Não foi possível adicionar as falas: " + ex.Message, "Voz GAT", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
    ...
    private void VoiceRestoreBackup167()
    {
        ...
        {
            MessageBox.Show("Ainda não existe backup de voz. O backup é criado automaticamente antes de uma substituição.", "Voz GAT", MessageBoxButtons.OK, MessageBoxIcon.Information);
            return;
        }

        if (MessageBox.Show("Restaurar o pacote de voz salvo antes da última substituição?", "Voz GAT", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) return;
        try
        {
            VoiceStop062();
            string active = VoiceIndividualRoot166();
        ...
        {
            MessageBox.Show("Não foi possível restaurar o backup: " + ex.Message, "Voz GAT", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
...
'@
    $voiceText = $voiceText.Replace($insertBefore, $manager + $insertBefore)
}

# Falas extras entram no mesmo sorteio de cada evento.
$patternGroup = '(?s)    private bool VoicePlayGroup062\(string group, bool interrupt = true\)\s*\{.*?\r?\n    \}\r?\n\r?\n    private bool VoicePlayId062'
$newGroup = @'
    private bool VoicePlayGroup062(string group, bool interrupt = true)
    {
        ...
        int total = baseCount + extras167.Length;
        int pick = _voiceRandom062.Next(0, Math.Max(1, total));
    ...
    private bool VoicePlayId062
'@
$replacedGroup = [regex]::Replace($voiceText, $patternGroup, $newGroup, 1)
if ($replacedGroup -eq $voiceText) { throw 'VoicePlayGroup062 nao encontrado para 1.0.67.' }
$voiceText = $replacedGroup
...
Write-Host 'GAT Telemetria 1.0.67 TESTE: gerenciador de pacotes, substituicao, falas extras, modelo TXT e backup.'
