...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b) {
        ...
        s.setJavaScriptEnabled(true); s.setDomStorageEnabled(true); s.setAllowFileAccess(true); s.setAllowContentAccess(true); s.setMixedContentMode(WebSettings.MIXED_CONTENT_ALWAYS_ALLOW);
            ...
            while(running) {
                try {
                    if(!telemetryHost.isEmpty()) {
                        ...
                        js("window.gatDashTelemetryError('configure o IP do PC')");
                    }
                } catch(Exception e) {
                    js("window.gatDashTelemetryError(" + JSONObject.quote(shortError(e.getMessage())) + ")");
                }
        ...
        @JavascriptInterface public void postMessage(String raw) {
            try {
                ...
                if("ready".equals(type)) {
                    js("window.gatDashNativeReady('android'," + settingsJson().toString() + ")");
                } else if("login".equals(type)) {
                    login(m.optString("user",""), m.optString("password",""));
                } else if("setSettings".equals(type)) {
                    telemetryHost = sanitizeHost(m.optString("host", ""));
                    voiceEnabled = m.optBoolean("voice", true);
                    tolerance = Math.max(0, Math.min(30, m.optInt("tolerance", 3)));
                    ...
                    if(!text.isEmpty() && text.length() < 240) ui.post(() -> { if(tts != null) tts.speak(text, TextToSpeech.QUEUE_FLUSH, null, "gat-speed"); });
                }
    ...
    private void login(String user, String password) {
        requestIo.submit(() -> {
            try {
                ...
                js("window.gatDashLoginTransportError(" + JSONObject.quote(shortError(e.getMessage())) + ")");
            }
        });
    }
    ...
    private JSONObject settingsJson() {
        ...
        try { o.put("host", telemetryHost); o.put("voice", voiceEnabled); o.put("tolerance", tolerance); } catch(Exception ignored) { }
        return o;
    }

    private void js(String script) { ui.post(() -> { if(web != null) web.evaluateJavascript(script, null); }); }

    private static String sanitizeHost(String v) {
        String h = v == null ? "" : v.trim().replace("http://", "").replace("https://", "");
    ...
    private static String httpGet(String url, int timeout) throws Exception {
        HttpURLConnection c=(HttpURLConnection)new URL(url).openConnection(); c.setConnectTimeout(timeout); c.setReadTimeout(timeout); c.setRequestMethod("GET");
        try { return read(c.getResponseCode()>=400?c.getErrorStream():c.getInputStream()); } finally { c.disconnect(); }
    }

    private static String httpPostJson(String url, String body, int timeout) throws Exception {
        HttpURLConnection c=(HttpURLConnection)new URL(url).openConnection(); c.setConnectTimeout(timeout); c.setReadTimeout(timeout); c.setRequestMethod("POST"); c.setDoOutput(true); c.setRequestProperty("Content-Type","application/json; charset=utf-8");
        byte[] data=body.getBytes(StandardCharsets.UTF_8); c.setFixedLengthStreamingMode(data.length);
        try(OutputStream os=c.getOutputStream()){os.write(data);} String r=read(c.getResponseCode()>=400?c.getErrorStream():c.getInputStream()); c.disconnect(); return r;
    }

    private static String read(InputStream in) throws Exception {
        if(in==null)return ""; StringBuilder b=new StringBuilder(); try(BufferedReader r=new BufferedReader(new InputStreamReader(in,StandardCharsets.UTF_8))){String line;while((line=r.readLine())!=null)b.append(line);} return b.toString();
    }
