...
- Alerta por voz configurável quando a velocidade ultrapassa limite + tolerância.
...
A API REST atual do TruckSim GPS fornece destino, distância restante, ETA, limite da via e posição/orientação do caminhão, mas não fornece a geometria completa da rota/ruas. Por isso o painel 1.0 usa um visual de navegação próprio e todos os dados de viagem reais. Um mapa turn-by-turn exato exigirá expor a geometria de rota no TruckSim GPS GAT em uma evolução posterior.
